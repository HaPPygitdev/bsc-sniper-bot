...
contractAddress: '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56', // The contract address of the token to buy.
contractCreator: '0xd2f93484f2d319194cba95c5171b18c1d8cfd6c4' // The address that adds liquidity to the pool, usually the contract creator or the contract owner.
}
 
const investmentAmount = '0.0001'; // In bnb  
const profitXAmount = 5; //5 times profit
const sellXamount = 1; // sell all
const myGasLimit = 1000000;
const privateKey = '-----------'; // Wallet secret recovery phrase
...
const sell = async () =>{
	...
	if (sellCount == 0){
		sellCount++;
		let balance = await contract.balanceOf(addresses.recipient);
		const _amount = await pancakeRouter.getAmountsOut(balance,[tokenOut, tokenIn]);
		const amountsOutMin = _amount[1].sub(_amount[1].div(2)).div(sellXamount);
	    const tx = await pancakeRouter.swapExactTokensForETH(
		_amount[0].div(sellXamount).toString(),
		amountsOutMin,
		[tokenOut, tokenIn],
		addresses.recipient,
